...
import { httpRequestsTotal, pushWebRTCStats, deleteSession, listSessions, registerMetricsEndpoint } from "./MetricsServer";
...
export function createWebUIServerExpressApp(
	...
	webExpressApp.use(express.json());

	webExpressApp.use((_req: express.Request, res: express.Response, next: express.NextFunction) => {
		res.on("finish", () => {
			httpRequestsTotal.inc({ method: _req.method, status_code: String(res.statusCode) });
		});
		next();
	});
	...
	webExpressApp.post("/api/metrics/push", (req: express.Request, res: express.Response) => {
		const { id_robot, session_id, stats } = req.body;
		if (
			typeof id_robot === "string" && isValidObjectId(id_robot) &&
			typeof session_id === "string" && isValidUUID(session_id) &&
			Array.isArray(stats)
		) {
			pushWebRTCStats(id_robot, session_id, stats);
		}
		res.status(204).end();
	});

	webExpressApp.get("/api/metrics/sessions", (_req: express.Request, res: express.Response) => {
		res.json(listSessions());
	});

	webExpressApp.delete("/api/metrics/session/:sessionId", (req: express.Request, res: express.Response) => {
	...
	if (config.metrics.enabled) {
		registerMetricsEndpoint(webExpressApp, config.metrics.path);
	}
